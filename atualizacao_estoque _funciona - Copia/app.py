...
def buscar_dados_firebird(cod_cliente, data_inicial, data_final):
    ...
    SELECT 
        ...
        C.NOME,
        P.COD_PRODUTO,
        P.DESCRICAO1 AS DESCRICAO_PRODUTO,
        C2.COD_COR,
        C2.DESCRICAO AS DESCRICAO_COR,
        PE.TAMANHO,
...
def buscar():
    ...
    return render_template('formulario.html', df=df, titles=df.columns.values, filename='dados.xlsx', cod_cliente=cod_cliente, data_inicial=data_inicial, data_final=data_final)
...
def detalhes_produto():
    ...
    FROM MOVIMENTO M
    LEFT JOIN PRODUTOS_EVENTOS PE ON (M.COD_OPERACAO=PE.COD_OPERACAO AND M.TIPO_OPERACAO=PE.TIPO_OPERACAO)
    LEFT JOIN CLIENTES C ON M.CLIENTE = C.CLIENTE
...
def salvar(filename):
    ...
    descricao_produto = dados_formulario.get('descricao_produto', [])
    cod_cor = dados_formulario.get('cod_cor', [])
    descricao_cor = dados_formulario.get('descricao_cor', [])
