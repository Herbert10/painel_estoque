...
def selecionar_arquivo():
    root = tk.Tk()
    root.withdraw()
    return filedialog.askopenfilename(title="Selecione o arquivo de remessa", filetypes=[("Arquivos REM", "*.rem")])


def selecionar_diretorio():
    root = tk.Tk()
    root.withdraw()
    return filedialog.askdirectory(title="Selecione onde salvar os boletos")


def formatar_data(data_vencimento):
    """Converte data de DDMMYY para DD/MM/YYYY"""
    try:
        data_formatada = datetime.datetime.strptime(data_vencimento, "%d%m%y").strftime("%d/%m/%Y")
...
def calcular_digito_verificador(codigo):
    ...
    soma = sum(int(num) * pesos[i % len(pesos)] for i, num in enumerate(reversed(codigo)))
    resto = soma % 11
    dv = 11 - resto
...
def gerar_codigo_barras(boleto):
    """Gera um código de barras válido para o boleto bancário"""
    try:
        ...
        # Código final precisa ter exatamente 44 dígitos
        codigo_final = codigo_base[:4] + dv + codigo_base[4:]

        # Verifica se o código tem 44 caracteres e apenas números
        if len(codigo_final) != 44 or not codigo_final.isdigit():
            print(f"Erro: Código de barras inválido: {codigo_final}")
...
def gerar_pdf_boleto(boleto, pasta_destino):
    """Gera um boleto bancário em formato PDF"""
    try:
        ...
        c.drawString(20 * mm, 240 * mm, f"Vencimento: {formatar_data(boleto['vencimento'])}")
        ...
        # Verifica se o código de barras tem 44 dígitos e é numérico
        if len(codigo_barras) == 44 and codigo_barras.isdigit():
            # Se o número for ímpar, adiciona um zero no final para garantir que seja par
            if len(codigo_barras) % 2 != 0:
                codigo_barras += "0"
            ...
            print(f"Erro: Código de barras inválido e não será gerado: {codigo_barras}")
...
def ler_arquivo_remessa(caminho_arquivo):
    """Lê o arquivo .rem e extrai os dados para gerar boletos"""
    boletos = []
        ...
        for linha in arquivo:
            if linha.startswith("1"):  # Registro de transação
                boleto = {
                    ...
                    "valor": float(linha[127:139].strip().zfill(10)) / 100,
