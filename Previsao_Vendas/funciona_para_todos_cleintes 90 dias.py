...
# Consulta SQL para buscar clientes PJ com histórico de compras adequado
query = """
...
# Criar DataFrame
colunas = ["cod_cliente", "nome_cliente", "vendedor_cadastro", "data_ultima_compra"]
...
for _, row in df.iterrows():
    ...
    prompt_email = f"""
    Gere uma mensagem formal para o cliente {nome}, mencionando que faz mais de 90 dias desde a última compra ({data_ultima_compra}), mas que ele esteve ativo no último ano. A mensagem deve ser cordial e incentivar uma nova compra. Assine como {vendedor}.
    """
    mensagens_email.append(gerar_mensagem(prompt_email))

    prompt_whatsapp = f"""
    Gere uma mensagem curta e informal para WhatsApp para o cliente {nome}, mencionando que faz mais de 90 dias desde a última compra ({data_ultima_compra}), mas que ele esteve ativo no último ano. Use tom amigável e incentive uma nova compra. Assine como {vendedor}.
    """
